classdef lasdata < handle
    ...
    properties
        ...
        skipper= 100; %Number of values skipped (defaults to 100) 
    end
...
methods (Access=public)
    ...
    function obj = lasdata(filename,skip,command)
    ...
    % however this method is slower as it causes reloading of the data.
        ...
        if isempty(obj.selection)
            obj.selection = true(floor(obj.header.number_of_point_records/obj.skipper),1); 
        end
    ...
    function obj = read_xyz(obj,donotfilter)
        ...
        SKIP = obj.skipper*LEN-4;%(LEN - 4);
        POINTS = floor(obj.header.number_of_point_records/obj.skipper);
        OFFSET = obj.header.offset_to_point_data;      
        fseek(fid,OFFSET,-1);
        fprintf('get xyz. Number of Points: %d File Length: %d\n', POINTS,LEN)
        obj.x = fread(fid,POINTS,'*int32',SKIP);  %x
        fprintf('got x\n')
        fseek(fid,OFFSET+4,-1);
        obj.y = fread(fid,POINTS,'*int32',SKIP);  %y
        fprintf('got y\n')
        fseek(fid,OFFSET+8,-1);
        obj.z = fread(fid,POINTS,'*int32',SKIP);  %z  
        fprintf('got z\n')
